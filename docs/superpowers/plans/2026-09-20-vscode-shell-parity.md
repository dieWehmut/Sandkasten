...
- [ ] Generate the extension, file-name, and folder-name maps plus the inlined
      SVG table from the pinned upstream manifests.
- [ ] Resolve a file or folder path to its upstream icon id, falling back to the
      neutral default for unknown names.
- [ ] Render the real glyph in the explorer, tabs, and breadcrumbs for both
      themes, and keep the tree aligned with the previous icon box size.
- [ ] Cover extension, exact file name, folder name, and unknown-file cases.
- [ ] Commit the table, the component, and the call sites in focused slices.
...
- [ ] Show only the brand mark, drop the wordmark, and remove the Run menu from
      the native menu template while keeping run/stop reachable.
- [ ] Delete the connection status, setup, locale, history, inspector, and theme
      controls from the title row and their now-dead styles.
- [ ] Keep the window title, the native menus, and the drag region intact.
- [ ] Update the header, menu, and accessibility tests to the reduced row.

## Task 3: Back/forward history

Files: new history composable, `App.vue`, header arrows and styles, tests.

- [ ] Record the active editor path in a bounded history as files open.
- [ ] Render back and forward arrows next to the brand mark, disabled when no
      step is available, with localized labels and accelerators.
- [ ] Restore the target file from each arrow and assert the button state after
...
- [ ] Collect the real commands with their accelerators plus workspace files.
- [ ] Center the search control in the title row, open the palette with
      `Ctrl+E`/`Ctrl+P` and from the control, and close it with Escape or an
      outside click.
- [ ] Support keyboard navigation, run the highlighted command or open the
      highlighted file, and never list commands the app cannot perform.
- [ ] Track recently opened files and show them before any query.
...
- [ ] Replace the activity-bar setup button with a settings button that opens the
      settings view; keep the setup guide reachable from the native menu.
- [ ] Render the section sidebar with the search box and the appearance section
      from the reference: theme previews for system/light/dark, accent color,
      background, and foreground.
- [ ] Make the theme choice, accent, and background/foreground persist and apply
      to the running workbench, and add the missing `system` theme mode.
- [ ] Move the API endpoint and color scheme controls into the settings view so
      the removed header buttons keep an entry point.
- [ ] Filter sections from the settings search.
...
- [ ] Run the complete web, desktop, CLI, and layout gates plus the browser
      smoke check.
- [ ] Extend the desktop E2E for the reduced title row, the palette, the
