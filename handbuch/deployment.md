...
- The runner image includes Go plus toolchains/interpreters for Bash/Shell, C, Cangjie, C++, C#, Coq, Java, JavaScript, Julia, Kotlin, Lean4, Lua, PHP, Prolog, Python, R, Racket, Ruby, Rust, Scala, SQL/SQLite, Swift, TypeScript, and Zig execution.
...
The full smoke submits source through the HTTP API for `go`, `bash`, `cangjie`, `c`, `cpp`, `csharp`, `coq`, `java`, `javascript`, `julia`, `kotlin`, `lean4`, `lua`, `php`, `prolog`, `python`, `r`, `racket`, `ruby`, `rust`, `scala`, `sql`, `swift`, `typescript`, and `zig`.
