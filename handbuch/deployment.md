...
The full smoke submits source through the HTTP API for `go`, `bash`, `cangjie`, `clojure`, `c`, `cpp`, `csharp`, `coq`, `crystal`, `dart`, `elixir`, `fsharp`, `java`, `javascript`, `julia`, `kotlin`, `lean4`, `lua`, `mojo`, `nextflow`, `nim`, `perl`, `php`, `prolog`, `python`, `r`, `racket`, `ruby`, `rust`, `scala`, `sql`, `swift`, `typescript`, `wdl`, and `zig`.
