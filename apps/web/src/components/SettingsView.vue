...
const props = defineProps<{
  ...
  /** Which colors the user has overridden for the active theme (the "modified" marker). */
  customized?: Partial<Record<AppearanceColor, boolean>>;
}>();
...
<template>
  <section class="settings-view" data-testid="settings-view" :aria-label="t('settings.title')">
    ...
    <main class="settings-content">
      <!-- The search sits in the settings header, the way the settings editor's
           own search field does, with a rule under it before the rows. -->
      <header class="settings-header">
        <div class="settings-header__row">
          <h1 ref="heading" tabindex="-1">{{ current ? t(`settings.${current.id}`) : t('settings.title') }}</h1>
