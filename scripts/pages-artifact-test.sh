...
assert_workflow_contract() {
  ...
  grep -Fq 'cache-dependency-path: apps/web/package-lock.json' "$workflow" || { fail 'workflow must cache from apps/web/package-lock.json'; return 1; }
  ...
  grep -Fq 'cp -R apps/web/dist/. _site/' "$workflow" || { fail 'workflow must stage the complete built WebUI before config generation'; return 1; }
